---
title: "Chronic Illness and Depression: Prevalence Comorbidity & Costs"
author: "John James"
...
```{r preprocessData, echo = F, include=T,  results='html'}
brfss <- preProcessData(brfss2013)
```   

```{r univariateAnalysis, echo = F}
...
```{r rq2, include=T, echo = F, results='asis'}
...
`r figr("chronic", TRUE, type="Table")`: Chronic conditions evaluated in this study
```{r chronic, echo = F, message = FALSE, warning=FALSE}
kable(chronic) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center")
```
...
`r figr("variables", TRUE, type="Table")`: BRFSS variables used in the study
...
### Power Analysis 
Variable and partial response rates may have affected sample size. To ensure that readers could rely upon non-significant results, this analyst conducted several power analyses using the pwr package [@Ekstrom2017]. Each test indicated the minimum sample size required to detect a small effect of 0.1, with the $p < .05$ probability of a type I error of p < .05 and power of 0.8. `r figr("power", TRUE, type="Table")` summarizes the minimum required sample size ($N$) for each type of statistical test to be conducted.

`r figr("power", TRUE, type="Table")`: Sample Size Estimates from Power Analysis
```{r power, include=T, echo = F, results='html'}
power <- estimateSampleSize()
kable(power, digits = 2) %>%  
...
Average valid response rate, as indicated in `r figr("categorical", TRUE, type="Table")`, was `r round(sum(univariate$categorical$Valid) / sum(univariate$categorical$Responses) * 100, 0)`%, for all variables.

`r figr("categorical", TRUE, type="Table")`: Summary of Univariate Analysis: Categorical Variables
```{r categorical, include=T, echo = F, results='html'}
kable(univariate$categorical, digits = 2) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
``` 

#### Depression Variable
Of the `r univariate$depression$summary[1,4]` responses, `r univariate$depression$summary[1,5]` were valid yielding a response rate of approximately `r round(univariate$depression$summary[1,7],0)`%.  As indicated in `r figr("analyzeDepressionPlot", TRUE, type="Figure")`, the prevalence of diagnoses of depression was `r round(univariate$depression$table[1,3]$Pct, 1)`%.
...
kable(univariate$depression$table, digits = 1) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") 
``` 
</div>
...
kable(univariate$chronic$table, digits = 1) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") 
``` 

</div>
`r figr("analyzeChronicPlot", TRUE, type="Figure")`: Frequency distribution of diagnoses of chronic illness
...
#### Income Variable
The 'Income' variable, a five level ordinal categorical variable, had a response rate of `r round(univariate$income$summary[1,7], 0)`%, leaving a total sample size of `r univariate$income$summary[1,5]`. The frequency distribution is summarized in `r figr("analyzeIncomePlot", TRUE, type="Figure")`.
...
kable(univariate$income$table, digits = 1) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") 
``` 

</div>
`r figr("analyzeIncomePlot", TRUE, type="Figure")`: Frequency distribution of income levels

#### Education Variable
The 'Education' variable, a four level ordinal categorical variable, had a response rate of approximately `r round(univariate$education$summary[1,7], 0)`%, leaving a total sample size of `r univariate$education$summary[1,5]`. The frequency distribution is summarized in `r figr("analyzeEducationPlot", TRUE, type="Figure")`.
...
kable(univariate$education$table, digits = 1) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") 
``` 
</div>
`r figr("analyzeEducationPlot", TRUE, type="Figure")`: Frequency distribution of education level completed
...
The combined response rate of the Depression and Income variables was `r round(rq1$tables$incomeFreq[6,3] / nrow(brfss) * 100, 1)`%, yielding `r rq1$tables$incomeFreq[6,3]` completed cases for analysis. `r figr("rq1IncomeFreqTable", TRUE, type="Table")` and `r figr("rq1IncomePropTable", TRUE, type="Table")` show the frequencies and proportions, respectively, of depression diagnoses for each income level.

<center>
`r figr("rq1IncomeFreqTable", TRUE, type="Table")`: Marginal contingency table of frequencies of depression diagnoses by income level
```{r rq1IncomeFreqTable, include=T, echo = F, results='asis'}
stargazer(format(rq1$tables$incomeFreq, quote = FALSE, justify = "right", big.mark = ","), type = 'html')
``` 
</center>

<center>
`r figr("rq1IncomePropTable", TRUE, type="Table")`: Marginal contingency table of proportions of depression diagnoses by income level
```{r rq1IncomePropTable, include=T, echo = F, results='asis'}
stargazer(format(rq1$tables$incomeProp, quote = FALSE, justify = "right", big.mark = ","), type = 'html')
``` 
</center>

Of those respondents who reported income, `r rq1$tables$incomeFreq[6,1]` (`r round(rq1$tables$incomeFreq[6,1] / rq1$tables$incomeFreq[6,3] * 100, 2)`%) indicated a prior diagnosis of depression. Stated in `r figr("rq1IncomePropTable", TRUE, type="Table")` and further clarified in `r figr("rq1IncomePlots", TRUE, type="Figure")`, rates of depression were inversely associated with income. 

```{r rq1IncomePlots, include=T, echo = F, results='html'}
grid.arrange(rq1$plots$incomeFreq, rq1$plots$incomeProp, ncol = 2)
``` 
`r figr("rq1IncomePlots", TRUE, type="Figure")`: Marginal frequencies and proportions of depression diagnoses by income level
...
```{r rq1IncomeMosaic, include=T, fig.height=10, fig.width=14, echo = F, results='html'}
  mosaic(rq1$tables$incomeTable, shade = TRUE, legend = TRUE,
...
Lastly, `r figr("rq1IncomeOdds", TRUE, type="Figure")` presents the odds ratios at each income level.  

`r figr("rq1IncomeOdds", TRUE, type="Table")`: The marginal odds ratios of a diagnosis of depression by income level 
```{r rq1IncomeOdds, include=T, echo = F, results='html'}
kable(rq1$analysis$incomeOdds, digits = 2) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
``` 
...
The combined response rate of the Depression and Education variables was `r round(rq1$tables$educationFreq[5,3] / nrow(brfss) * 100, 2)`%, yielding `r rq1$tables$educationFreq[5,3]` completed cases for analysis. `r figr("rq1EducationFreqTable", TRUE, type="Table")` and `r figr("rq1EducationPropTable", TRUE, type="Table")` show the frequencies and proportions of depression diagnoses for each education category.

<center>
`r figr("rq1EducationFreqTable", TRUE, type="Table")`: Marginal contingency table of frequencies of depression diagnoses by education level
```{r rq1EducationFreqTable, include=T, echo = F, results='asis'}
stargazer(format(rq1$tables$educationFreq, quote = FALSE, justify = "right", big.mark = ","), type = 'html')
``` 
</center>

<center>
`r figr("rq1EducationPropTable", TRUE, type="Table")`: Marginal contingency table of proportions of depression diagnoses by education level
```{r rq1EducationPropTable, include=T, echo = F, results='asis'}
stargazer(format(rq1$tables$educationProp, quote = FALSE, justify = "right", big.mark = ","), type = 'html')
``` 
</center>
...
The following bar charts provide graphical representations of the marginal frequencies and proportions of depression diagnoses by education category. `r figr("rq1EducationPlots", TRUE, type="Figure")` shows that the lowest and highest rates of depression were associated the highest and lowest levels of education; and, those that attended, but did not complete college or a trade school had diagnoses of depression at a slightly higher rate than those who had only completed high school.

```{r rq1EducationPlots, include=T, echo = F, results='html'}
grid.arrange(rq1$plots$educationFreq, rq1$plots$educationProp, ncol = 2, top = "Distribution of Diagnoses of Depression")
``` 
`r figr("rq1EducationPlots", TRUE, type="Figure")`: Marginal frequencies and proportions of depression diagnoses by education level completed
...
```{r rq1EducationMosaic, include=T, fig.height=10, fig.width=14, echo = F, results='html'}
  mosaic(rq1$tables$educationTable, shade = TRUE, legend = TRUE,
         labeling= labeling_border(rot_labels = c(0,0,0,0), 
                                   varnames = c(FALSE, TRUE),
...
Lastly, `r figr("rq1EducationOdds", TRUE, type="Figure")` presents the odds of a depression diagnosis at each level of education.

`r figr("rq1EducationOdds", TRUE, type="Table")`: The marginal odds ratios of a diagnosis of depression by education level 
```{r rq1EducationOdds, include=T, echo = F, results='html'}
kable(rq1$analysis$educationOdds, digits = 2) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
``` 
...
The conditional association of income and depression, controlling for education was significant at each level of education. In other words, rates of depression and income were inversely associated, regardless of education level, as `r figr("rq1InteractionPlots", TRUE, type="Figure")` presents.

```{r rq1InteractionPlots, include=T, echo = F, fig.height=10, results='html'}
grid.arrange(rq1$plots$conditionalProp[[1]], rq1$plots$conditionalProp[[2]],
...
``` 
`r figr("rq1InteractionPlots", TRUE, type="Figure")`: Conditional proportions of depression diagnoses by income, controlling for education

The mosaic plot in `r figr("rq1ConditionalMosaic", TRUE, type="Figure")` graphically depicts the association between depression diagnoses and income levels, controlling for education, vis-a-vis the independence model. 
...
lnames <- list(Income = c("< 15k", "< 25k", "< 35k", "< 50k", "50k +"),
  ...
  mosaic(rq1$tables$freq, shade = TRUE, legend = TRUE,
         labeling= labeling_border(rot_labels = c(0,0,0,0), 
                                   varnames = c(FALSE, TRUE),
                                   gp_labels = gpar(fontsize = 10),
                                   just_labels = c("left", 
...
<div class = 'col2'>
`r figr("rq1CMH1", TRUE, type="Table")`: Conditional association of depression and income. Stratum: Did not graduate high school
```{r rq1CMH1, include=T, echo = F, results='html'}
kable(rq1$tests$condInd[[1]]$df, digits = c(0,0,0,0,4), caption = 'Cochran–Mantel–Haenszel for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Did not graduate high school' = 5))
``` 

`r figr("rq1CMH2", TRUE, type="Table")`: Conditional association of depression and income. Stratum: Graduated high school
```{r rq1CMH2, include=T, echo = F, results='html'}
kable(rq1$tests$condInd[[2]]$df, digits = c(0,0,0,0,4), caption = 'Cochran–Mantel–Haenszel for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Graduated high school' = 5))
``` 

</div>

<div class = 'col2'>
`r figr("rq1CMH3", TRUE, type="Table")`: Conditional association of depression and income. Stratum: Attended college or technical school
```{r rq1CMH3, include=T, echo = F, results='html'}
kable(rq1$tests$condInd[[3]]$df, digits = c(0,0,0,0,4), caption = 'Cochran–Mantel–Haenszel for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Attended college or technical school' = 5))
``` 

`r figr("rq1CMH4", TRUE, type="Table")`: Conditional association of depression and income. Stratum: Graduated college or technical school
```{r rq1CMH4, include=T, echo = F, results='html'}
kable(rq1$tests$condInd[[4]]$df, digits = c(0,0,0,0,4), caption = 'Cochran–Mantel–Haenszel for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Graduated college or technical school' = 5))
``` 

</div>

As indicated in `r figr("rq1CMH1", TRUE, type="Table")`, `r figr("rq1CMH2", TRUE, type="Table")`, `r figr("rq1CMH3", TRUE, type="Table")`, and `r figr("rq1CMH4", TRUE, type="Table")`, the differences in proportions were significant, $p$ < .001, across each education level. To ascertain the strength of the associations, odds ratios were computed. `r figr("rq1ConditionaOdds1", TRUE, type="Table")`, `r figr("rq1ConditionaOdds2", TRUE, type="Table")`, `r figr("rq1ConditionaOdds3", TRUE, type="Table")`. and `r figr("rq1ConditionaOdds4", TRUE, type="Table")` list the conditional odds ratios at each level of education.  

<div class = 'col2'>
`r figr("rq1ConditionaOdds1", TRUE, type="Table")`: Conditional odds ratios of depression and income. Stratum: Did not graduate high school
```{r rq1ConditionaOdds1, include=T, echo = F, results='html'}
kable(rq1$analysis$conditionalOdds[[1]]$df, digits = 2, caption = 'Conditional Odds Ratios for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Did not graduate high school' = 2))
``` 

`r figr("rq1ConditionaOdds2", TRUE, type="Table")`: Conditional odds ratios of depression and income. Stratum: Graduated high school
```{r rq1ConditionaOdds2, include=T, echo = F, results='html'}
kable(rq1$analysis$conditionalOdds[[2]]$df, digits = 2, caption = 'Conditional Odds Ratios for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Graduated high school' = 2))
``` 
</div>

<div class = 'col2'>
`r figr("rq1ConditionaOdds3", TRUE, type="Table")`: Conditional odds ratios of depression and income. Stratum: Attended college or technical school
```{r rq1ConditionaOdds3, include=T, echo = F, results='html'}
kable(rq1$analysis$conditionalOdds[[3]]$df, digits = 2, caption = 'Conditional Odds Ratios for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Attended college or technical school' = 2))
``` 

`r figr("rq1ConditionaOdds4", TRUE, type="Table")`: Conditional odds ratios of depression and income. Stratum: Graduated college or technical school
```{r rq1ConditionaOdds4, include=T, echo = F, results='html'}
kable(rq1$analysis$conditionalOdds[[4]]$df, digits = 2, caption = 'Conditional Odds Ratios for Depression by Income') %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c('Stratum: Graduated college or technical school' = 2))
``` 
</div>
...
The combined response rate of the income, education, and depression variables was `r round(rq1$tables$interactionFreq[21,3] / nrow(brfss) * 100, 1)`%, yielding `r rq1$tables$interactionFreq[21,3]` completed cases for analysis. `r figr("rq1InteractionFreqTable", TRUE, type="Table")` and `r figr("rq1InteractionPropTable", TRUE, type="Table")` show the frequencies and proportions of depression diagnoses by the combination of income and education.

<center>
`r figr("rq1InteractionFreqTable", TRUE, type="Table")`: Contingency table of frequencies of depression diagnoses by income and education levels
```{r rq1InteractionFreqTable, include=T, echo = F, results='asis'}
stargazer(format(rq1$tables$jointFreq, quote = FALSE, justify = "right", big.mark = ","), type = 'html')
``` 
</center>

<center>
`r figr("rq1InteractionPropTable", TRUE, type="Table")`: Contingency table of proportions of depression diagnoses by income and education levels
```{r rq1InteractionPropTable, include=T, echo = F, results='asis'}
stargazer(format(rq1$tables$jointProp, quote = FALSE, justify = "right", big.mark = ",", digits = 3),  type = 'html')
``` 
</center>
...
lnames <- list(IncomeEducation = c("No High School (<15k)",
                                   ...
                                   "College Graduage(50k+)"))
  mosaic(rq1$tables$interactionTable, shade = TRUE, legend = TRUE,
         labeling= labeling_border(rot_labels = c(0,0,0,0), 
                                   varnames = c(FALSE, TRUE),
                                   gp_labels = gpar(fontsize = 10),
                                   ...
                                   set_labels = lnames))
``` 
`r figr("rq1InteractionMosaic", TRUE, type="Figure")`: Mosaic plot of joint association between depression and income and education

In `r figr("rq1InteractionMosaic", TRUE, type="Figure")`, blue indicates that the observed frequencies were greater than the expected frequencies under the independence mode, red means that the observed frequencies were fewer than the expected frequencies.
...
Odds ratios were calculated (`r figr("rq1JointOdds", TRUE, type="Table")`) to determine the strength of the association. 

`r figr("rq1JointOdds", TRUE, type="Table")`: Joint odds ratios of depression and income and education. 
```{r rq1JointOdds, include=T, echo = F, results='html'}
kable(rq1$analysis$interactionOdds %>% select(Income, Odds), digits = 2) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  group_rows("Did not graduate high school/Yes:No",1,5) %>%
  group_rows("Graduate high school/Yes:No",6,10) %>%
  group_rows("Attended college or technical school/Yes:No",11,15) %>%
  group_rows("Graduated college or technical school/Yes:No",16,19)
``` 
  ...
  mosaic(rq2$tables$r2, shade = TRUE, legend = TRUE,
         labeling = labeling_border(rot_labels = c(0,0,0,0),
                                   ...
                                   just_labels = c("left", 
                                                   ...
                                                   "right")))
``` 
`r figr("rq2Mosaic", TRUE, type="Figure")`: Mosaic plot of depression and chronic illness association
...
What is the effect of chronic disease on productivity? `r figr("rq3ChronicStats", TRUE, type="Table")` exhibits a `r round(rq3$stats$chronic$Mean[1] - rq3$stats$chronic$Mean[2], 2)` day difference in the mean number sick days. The following analysis explores the significance and effect of this difference.

`r figr("rq3ChronicStats", TRUE, type="Table")`: Descriptive Statistics of Restricted Activity Days by Chronic Disease Diagnosis 
```{r rq3ChronicStats, echo = F, message = FALSE, warning=FALSE}
kable(rq3$stats$chronic) %>%  
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center")
```

The frequency and proportion tables in `r figr("rq3ChronicBar", TRUE, type="Figure")` cite a total of `r sum(rq3$tables$chronicFreqTbl[,2])` respondents who reported one or more sick days, of that number `r round(sum(rq3$tables$chronicFreqTbl[1,2]) / sum(rq3$tables$chronicFreqTbl[,2]) * 100, 1)`% reported a diagnosis of chronic illness. Respondents with a diagnosis of chronic illness were `r round(((rq3$tables$chronicFreqTbl[1,2] / rq3$tables$chronicFreqTbl[1,1]) / (rq3$tables$chronicFreqTbl[2,2] / rq3$tables$chronicFreqTbl[2,1])), 2) ` times as likely to have one or more sick days in a month than those without a diagnosis of chronic illness.   

```{r rq3ChronicBar, include=T, echo = F, results='html'}
grid.arrange(rq3$plots$chronicFreqBar, rq3$plots$chronicPropBar, ncol = 2, top = 'Chronic Illness and Reports of One or More Sick Days')
``` 
`r figr("rq3ChronicBar", TRUE, type="Figure")`: Chronic Illness and Reports of Health-Related Restricted Activity

```{r rq3ChronicBox, include=T, echo = F, results='html'}
grid.arrange(rq3$plots$chronicViolin, rq3$plots$chronicBox, ncol = 2, top = "Distribution Summary of Reports of Sick Days by Diagnosis of Chronic Illness")
``` 
`r figr("rq3ChronicBox", TRUE, type="Figure")`: Distribution Summary of Reports of Sick Days by Diagnosis of Chronic Illness

The violin and box plots in `r figr("rq3ChronicBox", TRUE, type="Figure")` show two different distributions, one characterized by a relatively wide spread, the other centered around its median of zero days. The group reporting chronic illness reported up to ten sick days, 75% of the time; whereas, the other group reported no more than two sick days, 75% of the time. 

```{r rq3ChronicHist1, include=T, echo = F, results='html'}
rq3$plots$chronicHist1
``` 
`r figr("rq3ChronicHist2", TRUE, type="Figure")`: Distribution of Sick Days by Diagnosis of Chronic Illness

As evidenced by `r figr("rq3ChronicBox", TRUE, type="Figure")`, both distributions peak at zero days, in fact, zero days was not only the mean but the mode for this measure. `r figr("rq3ChronicHist1", TRUE, type="Figure")` shows a magnified view of the distribution, created by removing reports of zero days. The group with the diagnoses had consistently higher frequencies of longer bouts of restricted activity. 
```{r rq3ChronicHist2, include=T, echo = F, results='html'}
rq3$plots$chronicHist2
``` 
`r figr("rq3ChronicHist2", TRUE, type="Figure")`: Distribution of One or More Sick Days by Diagnosis of Chronic Illness
...
The results of the tests are summarized in `r figr("rq3Pairwise", TRUE, type="Table")`.

`r figr("rq3Pairwise", TRUE, type="Table")`: Pairwise Mann-Whitney-Wilcoxon tests of depression and chronic illness on productivity
```{r rq3Pairwise, echo = F, message = FALSE, warning=FALSE}
kable(rq3$tests$pairwise) %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c(" " = 1, "Group A" = 2, "Group B" = 2, " " = 6))
```
...
```
`r figr("rq3AllChronicPlot", TRUE, type="Figure")`: Sick days during preceding 30 days by chronic condition
...
Considering the marginal effects of diagnoses of one or more chronic conditions, `r figr("rq4ChronicStats", TRUE, type="Table")` shows that those with chronic illness diagnoses reported, on average `r round((rq4$stats$chronic$Mean[1] / rq4$stats$chronic$Mean[2] * 100) - 100, 1)`% greater number of Dr. visits in the 12 months preceding the survey. The median number of Dr. visits for those with a chronic illness diagnosis was `r rq4$stats$chronic$Median[1]` versus those without a diagnosis, `r rq4$stats$chronic$Median[2]`, a `r round((rq4$stats$chronic$Median[1] / rq4$stats$chronic$Median[2] * 100) - 100, 1)`% difference. 

`r figr("rq4ChronicStats", TRUE, type="Table")`: Descriptive Statistics of effect of chronic illness on the number of Dr. visits in a 12 month period
```{r rq4ChronicStats, echo = F, message = FALSE, warning=FALSE}
kable(rq4$stats$chronic) %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") 
```
As indicated in `r figr("rq4ChronicBox", TRUE, type="Figure")`, the group with chronic illness diagnoses had a higher number of reports of five or more Dr. visits in the year preceding the survey. The reports of Dr. visits among those without a diagnosis had a lower and wider center of around `r rq4$stats$chronic$Median[2]` visits for the year.


```{r rq4ChronicBox, include=T, echo = F, results='html'}
grid.arrange(rq4$plots$chronicViolin, rq4$plots$chronicBox, ncol = 2, top = "Chronic Illness and Summary Distribution of Dr. Visits")
```
`r figr("rq4ChronicBox", TRUE, type="Figure")`: Chronic Illness and Summary of Distribution of Dr. Visits

`r figr("rq4ChronicHist1", TRUE, type="Figure")`, visually represents the quantitative results above. The group with diagnoses of chronic illness reported five or more Dr. visits,  as frequently or more frequently than those without a diagnosis (`r figr("rq4ChronicHist2", TRUE, type="Figure")`). 

```{r rq4ChronicHist1, include=T, echo = F, results='html'}
rq4$plots$chronicHist1
```
`r figr("rq4ChronicHist1", TRUE, type="Figure")`: Distribution of Dr. Visits by Diagnosis of Chronic Illness

```{r rq4ChronicHist2, include=T, echo = F, results='html'}
rq4$plots$chronicHist2
```
`r figr("rq4ChronicHist2", TRUE, type="Figure")`: Distribution of five or more Dr. Visits by Diagnosis of Chronic Illness
...
The results of the tests are summarized in `r figr("rq4Pairwise", TRUE, type="Table")`.

`r figr("rq4Pairwise", TRUE, type="Table")`: Pairwise Mann-Whitney-Wilcoxon tests of depression and chronic illness on use of health services
```{r rq4Pairwise, echo = F, message = FALSE, warning=FALSE}
kable(rq4$tests$pairwise) %>% 
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, position = "center") %>%
  add_header_above(c(" " = 1, "Group A" = 2, "Group B" = 2, " " = 6))
```
...
```
`r figr(" rq4AllChronicPlot", TRUE, type="Figure")`: Dr. visits during previous 12 months, by chronic condition
